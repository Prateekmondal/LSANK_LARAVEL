    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.getElementById('applyWellNoMatch').addEventListener('click', function () {
                ...
                updateJobNoFromUnit(jcrData.unitNo || document.getElementById('id_unitNo')?.value);
                updateCableInfoFrom(jcrData.unitNo || document.getElementById('id_unitNo')?.value, jcrData.cableSize || document.getElementById('id_cableSize')?.value);
                var wellNoModalElement = document.getElementById('wellNoMatchModal');
                bootstrap.Modal.getInstance(wellNoModalElement).hide();
            });

            function updateJobNoFromUnit(unitNo) {
                if (!unitNo) {
                    return;
                }

                fetch(`{{ route('ajax.getjobno') }}?unitNo=${encodeURIComponent(unitNo)}`)
                    .then(response => response.json())
                    .then(jobNo => {
                        var jobNoInput = document.getElementById('id_jobNo');
                        if (jobNoInput) {
                            jobNoInput.value = jobNo;
                        }
                    })
                    .catch(err => {
                        console.error('JobNo fetch failed', err);
                    });
            }

            function updateCableInfoFrom(unitNo, cableSize) {
                if (!unitNo || !cableSize) {
                    return;
                }

                fetch(`{{ route('ajax.getcableinfo') }}?unitNo=${encodeURIComponent(unitNo)}&cableSize=${encodeURIComponent(cableSize)}`)
                    .then(response => response.json())
                    .then(data => {
                        ...
                        var fieldMap = {
                            shoeDate: 'id_shoeDate',
                            weakPoint: 'id_weakPoint',
                            cableLength: 'id_cableLength',
                            initialLength: 'id_initialLength'
                        };
                    ...
                    .catch(err => {
                        console.error('Cable info fetch failed', err);
                    });
            }

            var cableSizeInput = document.getElementById('id_cableSize');
            var unitNoInput = document.getElementById('id_unitNo');
            function refreshCableInfo() {
                if (!unitNoInput || !cableSizeInput) {
                    return;
                }
                updateCableInfoFrom(unitNoInput.value, cableSizeInput.value);
            }
            if (cableSizeInput) cableSizeInput.addEventListener('change', refreshCableInfo);
            if (unitNoInput) unitNoInput.addEventListener('change', refreshCableInfo);

            function fillFormFromJcr(jcrData) {
                var excludedFieldsets = ['timeinfo', 'logsrecorded', 'swc', 'explosiveinfo', 'hseinfo'];
                var excludedNames = ['time_register_id', 'selected_time_register_id', 'final_submit', 'creator_id', 'creator_signature', 'creator_signed_at', 'party_chief_id', 'party_chief_signature', 'party_chief_signed_at', 'operation_incharge_id', 'operation_incharge_signature', 'operation_incharge_signed_at', 'status', 'sap_document_number', 'sap_pushed_at', 'sap_status', 'shoeDate', 'weakPoint', 'cableLength', 'initialLength'];
