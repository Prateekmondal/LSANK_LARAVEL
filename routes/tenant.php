...
// Register a custom Livewire update route that runs INSIDE the tenancy
// middleware stack. Without this, Livewire AJAX calls (e.g. Filament Shield
// permission save) go to /livewire/update which only has the 'web' middleware
// — tenancy is never initialized, so Role/Permission queries hit the central
// DB and fail with "Table 'lsank_laravel.roles' doesn't exist".
// We use InitializeTenancyByDomainOrSkipForCentral so that:
//   - Central domain requests (localhost/admin): tenancy is skipped → central DB
//   - Tenant subdomain requests: tenancy is initialized → tenant DB
Livewire::setUpdateRoute(function ($handle) {
    return Route::post('/livewire/update', $handle)
        ->middleware([
            'web',
            \App\Http\Middleware\InitializeTenancyByDomainOrSkipForCentral::class,
        ])
...
Route::middleware([
    ...
    Route::get('/', function () {
        // tenancy is initialized by middleware for tenant subdomains
        $tenant = tenant('id');

        // Aggregate stats for the current tenant only (tenant DB)
        $loggingTypes = [
            'OH' => 0,
            'CH' => 0,
            'PL' => 0,
        ...
        try {
            // limit aggregation to current financial year (Apr 1 -> Mar 31)
            ...
            // if tenant DB doesn't have expected tables, leave zeros
        }
